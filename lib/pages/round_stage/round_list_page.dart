...
import 'package:bolalucu_league/pages/round_stage/round_leg_detail.dart';
import 'package:flutter/material.dart';

class RoundListPage extends StatefulWidget {
  const RoundListPage({ Key? key }) : super(key: key);

  @override
  _RoundListPageState createState() => _RoundListPageState();
}
...
class _RoundListPageState extends State<RoundListPage> {
  ...
  Future<dynamic> checkPhase() async {
    ...
    var data = await AppHelper.isOpenPhase(phaseID: 2);
    if (data['success']) {
      if (mounted) {
        setState(() {
          _isLoading = false;
        ...
        if (_isOpen!) {
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              title: Text("Round of 16", style: TextStyle(color: blackColor),),
            ),
            body: SafeArea(
              child: Container(
                      ...
                      child: InkWell(
                        onTap: (){
                          Navigator.of(context).push(
                            MaterialPageRoute(builder: (context) => RoundLegDetailPage(title: "${index+1}",))
                          );
                        },
                        child: Ink(
                          decoration: BoxDecoration(
